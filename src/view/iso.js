...
(function (root) {
    ...
    terrain.forEach(function (r) {
      ...
      for (var j = j0; j <= j1; j++) {
        for (var i = i0; i <= i1; i++) {
          ...
          if (f.tint) {
            var tc = hex3(r.kind === 'rocks' && ROCKTINT || f.tint), tw2 = Math.min(1, f.tw * (out < 0 ? 1 : Math.max(0, 1 - out / f.r) * (0.7 + vnoise(x / 1.6, y / 1.6, seed + 630) * 0.6)));
  ...
  function floorOf(kind) { return kind === 'rocks' && ROCKF || FLOOR[kind]; }
  ...
  function drawProp(g, pr, lift, cutaway) {
    ...
    switch (pr.kind) {
      ...
      case 'crystals': {
        ...
        ellipse(g, p.x, p.y, a(2.4 * sz7), a(1.1 * sz7), 'rgba(80,255,150,.10)');
        ellipse(g, p.x, p.y, a(1.4 * sz7), a(0.6 * sz7), 'rgba(120,255,180,.18)');
        var shards = [];
