...
(function (root) {
  ...
  /* ---------- the men themselves: names, ranks, and casualties ----------
     ...
     a bug unit only counts the biomass it loses. */
  ...
  function crewed(u) {
    if (u.faction === 'bugs') return false;
  ...
  function musterMen(u, carried, taken) {
    if (u.faction === 'bugs') { u.men = []; u.seenModels = standing(u); u.lostModels = 0; return u.men; }
  ...
  function syncMen(u, turn, taken) {
    if (u.faction === 'bugs') { countBiomass(u); return []; }
    if (!u.men) return [];
  ...
  /* The swarm's losses, as a running count: every model that goes down adds
     to it, and the ones the Endless Tide digs back out do not take it away. */
  function countBiomass(u) {
    var now = standing(u);
  ...
  /* What a lost bug is worth in biomass: its Tier for every model, and 25 for
     ...
     Queen is a little more than a whole Tier IV brood. */
  function biomassOf(p) { return !p ? 1 : isOvergrown(p) ? 25 : Math.max(1, p.tier || 1); }
  ...
  root.PMC = {
    ...
    soldierName: soldierName, rankFor: rankFor, crewed: crewed, musterMen: musterMen, syncMen: syncMen, survivors: survivors, biomassOf: biomassOf
  };
})(window);
