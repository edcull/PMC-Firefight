...
(function (root) {
  ...
  function assault(state, a, t, opts) {
    ...
    function martyr(u, foe) {
      ...
      // "the Rebel commander may order" it (p. 112): the player's standing order, and never the last two men
      if (state.orders && state.orders[u.side] && state.orders[u.side].martyr === false) return;
      if (u.models <= 2) return;
      var p = BY_KEY[u.key];
