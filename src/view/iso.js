...
(function (root) {
  ...
  function drawXenoMachine(g, u, opts) {
    ...
    function drawSkimmer() {
      ...
      function fin(sd) {
        var fh = m(sk.small ? 5 : 7);
        var pts = [S(-0.45 * L, sd * 0.26 * Wd2, zd - m(0.5)), S(-0.9 * L, sd * 0.26 * Wd2, zd - m(0.5)), S(-1.12 * L, sd * 0.4 * Wd2, zd + fh), S(-0.86 * L, sd * 0.38 * Wd2, zd + fh * 0.92)];
        path(pts, finSide(sd) > 0 ? WH.md : WH.dk); stroke(pts, 1, WH.seam, true);
        stroke([pts[0], pts[3]], m(0.7), GLO.m);                     // its leading edge in the army's colour
        if (!dead) ellipse(g, pts[3][0], pts[3][1], m(0.7), m(0.6), GLO.l);
      }
      // a wing: a thin plate swept back from the hull's flank, its leading edge in the army's colour
      var wz = zb + H * 0.3;
      function wingPlate(sd) {
        var top = [S(0.35 * L, sd * 0.3 * Wd2, wz), S(-0.52 * L, sd * Wd2, wz - m(0.8)), S(-0.86 * L, sd * 0.96 * Wd2, wz - m(0.8)), S(-0.72 * L, sd * 0.34 * Wd2, wz)];
        ...
        stroke([S(0.1 * L, sd * 0.36 * Wd2, wz), S(-0.6 * L, sd * 0.82 * Wd2, wz - m(0.6))], 1, WH.dk);   // a panel line
        stroke([top[0], top[1]], m(0.8), GLO.m);
        if (!dead) { stroke([top[0], top[1]], m(0.3), GLO.l); ellipse(g, top[1][0], top[1][1], m(0.9), m(0.8), GLO.l); }
        return top;
      }
      var wingTips = {};
      [-1, 1].forEach(function (sd) { if (finSide(sd) < 0) { wingTips[sd] = wingPlate(sd)[1]; fin(sd); } });
      ...
      [-1, 1].forEach(function (sd) { if (finSide(sd) >= 0) fin(sd); });
