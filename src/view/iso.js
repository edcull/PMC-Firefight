...
(function (root) {
  ...
  var HULL = {
    ...
    // the Light VTOL drone: a small flying disc with side pods and a long gun on top
      ...
      switch (st) {
        ...
        case 'disc': {
          ...
          part(0, 0, function () {
            ...
            // the gun: a long barrel on a low mount, laid along the top to one side
            shape(AF, [[R0 * 0.05, -R0 * 0.52], [R0 * 0.05, -R0 * 0.3], [-R0 * 0.35, -R0 * 0.3], [-R0 * 0.35, -R0 * 0.52]], z + Math.round(H * 0.32), 2, TB);
            noseGun(-R0 * 0.1, -R0 * 0.41, z + Math.round(H * 0.32) + 3, 0.62, 'mg');
          }, true);
          break;
        }
