...
(function (root) {
  ...
  var FLOOR = {
    ...
    // a desert world's crystal field: green glass grown out of the sand
    crystal: { ramp: ['#2b3e34', '#35503f', '#40614b', '#4d7358', '#5d8668'], fleck: ['#8fe3b3', '#d8f7e6', '#1a2a21'], rim: '#16241c' },
  ...
  var FIT = {
    ...
    crystal:  { r: 3, p: -0.1, d: 0.06, n: 0.04, tint: '#8fb59c', tw: 0.3 },
  ...
  function buildProps(terrain, objectives, seed, planet) {
    ...
    terrain.forEach(function (r, ri) {
      ...
      if (r.kind === 'crystal') {
        // clusters of green crystal standing up out of the glassy ground
        for (var cz = 0; cz < Math.max(3, Math.round(area / 6)); cz++) {
          var czq = spotIn(r, 0.5, rand);
          props.push({ kind: 'crystals', x: czq.x, y: czq.y, size: 1.3 + rand() * 1.1, seed: (rand() * 9999) | 0 });
        }
        return;
      }
  ...
  function drawProp(g, pr, lift, cutaway) {
    ...
    switch (pr.kind) {
      ...
      case 'crystals': {
        // three to six shards leaning out of one root, glassy green, lit on one face
        var cr7 = rng(pr.seed), n7 = 3 + (cr7() * 4 | 0), sz7 = pr.size || 1;
        shadowBlob(g, { x: p.x + a(0.8), y: p.y + a(0.35) }, a(1.6 * sz7), a(0.7 * sz7));
        ellipse(g, p.x, p.y, a(1.2 * sz7), a(0.5 * sz7), 'rgba(140,255,190,.14)');
        var shards = [];
        for (var k7 = 0; k7 < n7; k7++) {
          var lean = (cr7() - 0.5) * 0.8, hgt = a((1.6 + cr7() * 2.4) * sz7), wid = a((0.7 + cr7() * 0.5) * sz7);
          shards.push({ ox: (cr7() - 0.5) * a(1.4 * sz7), lean: lean, h: hgt, w: wid });
        }
        shards.sort(function (s1, s2) { return s1.h - s2.h; }).reverse().forEach(function (sh) {
          var bx7 = p.x + sh.ox, by7 = p.y, tx = bx7 + Math.sin(sh.lean) * sh.h, ty = by7 - Math.cos(sh.lean) * sh.h;
          // a hexagonal prism with a pointed cap: two side faces and a facet at the tip
          var cx7 = tx - Math.sin(sh.lean) * sh.w * 0.9, cy7 = ty + Math.cos(sh.lean) * sh.w * 0.9;
          poly(g, [[bx7 - sh.w, by7], [cx7 - sh.w * 0.8, cy7], [tx, ty], [bx7, by7 + sh.w * 0.35]], '#23704b');   // shaded face
          poly(g, [[bx7, by7 + sh.w * 0.35], [tx, ty], [cx7 + sh.w * 0.8, cy7], [bx7 + sh.w, by7]], '#4fbf82');   // lit face
          poly(g, [[cx7 - sh.w * 0.8, cy7], [tx, ty], [cx7 + sh.w * 0.8, cy7], [cx7, cy7 + sh.w * 0.3]], '#9df0c3'); // cap facet
          edgeLine(g, [bx7, by7 + sh.w * 0.35], [tx, ty], 'rgba(220,255,235,.75)', 1);         // the glint down the edge
          edgeLine(g, [bx7 - sh.w, by7], [cx7 - sh.w * 0.8, cy7], 'rgba(10,40,25,.7)', 1);
          dot(g, tx, ty, '#f0fff6', 1);
        });
        break;
      }
      case 'vent': {
