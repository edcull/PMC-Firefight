...
(function (root) {
  ...
  var HULL = {
    ...
    // the Light VTOL drone: a small flying disc with side pods and a sensor head, scanning
    vtoldrone: { len: 0.95, wid: 0.95, hgt: 6, fly: 2.8, craft: 'disc', gun: 0.6 },
  ...
  function drawMachineBody(g, u, opts) {
    ...
    function drawCraft() {
      ...
      switch (st) {
        ...
        case 'disc': {
          /* A small gun drone: a thin flat disc riding on its stand, a sensor
             pod hung off each side on a short arm, lights lit in their faces,
             and one long gun laid along the top. */
          ...
          if (!dead) {
            ...
            sEllipse(ug[0], ug[1], R0 * K * 0.6, R0 * K * 0.3, 'rgba(110,190,255,.14)');
            /* It is there to find and mark, not to fight: a scanning fan swept
               across the ground ahead, as the Xenotripod recon craft throws. */
            var tn = root.performance ? performance.now() : 0;
            var sw = Math.sin(tn / 700) * 0.3, reach = R0 + 0.55;
            var n0 = S3(AF(R0 * 0.97, 0), z + Math.round(H * 0.12));
          ...
          [-1, 1].forEach(function (sd) {
            part(0, sd * R0 * 1.1, function () {
              strut(AF, 0, sd * R0 * 0.7, 0, sd * R0 * 1.02, z + H * 0.1, 1.6);
              var pq0 = sd * R0 * 0.98, pq1 = sd * R0 * 1.32;
              shape(AF, [[R0 * 0.28, pq0], [R0 * 0.28, pq1], [-R0 * 0.22, pq1], [-R0 * 0.22, pq0]], z - H * 0.3, Math.round(H * 0.55), TB);
              if (!dead) [0.25, 0.75].forEach(function (f) {
                var lp = S3(AF(R0 * 0.29, pq0 + (pq1 - pq0) * f), z - H * 0.3 + H * 0.3);
                sEllipse(lp[0], lp[1], 0.9, 0.9, '#6ff0ff');
              });
            });
          });
          part(0, 0, function () {
            shape(AF, ring(R0 * 0.82), z, Math.round(H * 0.2), TB, null, ring(R0));                  // the underside, flaring out
            shape(AF, ring(R0), z + Math.round(H * 0.2), Math.round(H * 0.12), TB, null, ring(R0 * 0.94)); // the thin top face
            if (!dead) {
              for (var li = 0; li < 16; li += 4) {
                var la = li / 16 * Math.PI * 2 + Math.PI / 8, lp2 = S3(AF(Math.cos(la) * R0 * 0.99, Math.sin(la) * R0 * 0.99), z + Math.round(H * 0.22));
                sEllipse(lp2[0], lp2[1], 0.8, 0.6, 'rgba(150,215,255,.9)');
              }
            }
            // the sensor head on the front of the rim, where its marker beam leaves
            var sl = S3(AF(R0 * 0.97, 0), z + Math.round(H * 0.12));
            sEllipse(sl[0], sl[1], 2.4, 1.9, '#123a6e');
            sEllipse(sl[0], sl[1], 1.5, 1.2, dead ? '#3a4048' : '#6ebeff');
            if (!dead) sEllipse(sl[0] - 0.4, sl[1] - 0.4, 0.6, 0.5, '#e4f4ff');
          }, true);
          break;
        }
    ...
    function droneSpot() {
      ...
      if (spec.craft === 'disc') return along(-spec.len * 0.22, spec.wid * 0.22);
    ...
    function droneKit(only) {
      ...
      else if (spec.craft === 'disc') { back = -0.22; off = spec.wid * 0.22; y5 = lift + spec.hgt * 0.36; }   // on the top face, behind the gun
      ...
      var r = Math.max(3, K * 0.16) * (spec.craft === 'disc' ? 0.6 : 1), cy = dp.y - y5;
      if (only !== 'aerial') {
