...
(function () {
  ...
  function drawHUD() {
    ...
    function star(cx, cy, r) {
      ...
      for (var k = 0; k < 10; k++) {
        ...
        ctx[k ? 'lineTo' : 'moveTo'](cx + Math.cos(an) * rr, cy + Math.sin(an) * rr);
      }
      ctx.closePath(); ctx.fillStyle = '#f2c14e'; ctx.fill();
    }
    function heart(cx, cy, r) {
      ctx.beginPath();
      ctx.moveTo(cx, cy + r * 0.9);
      ctx.bezierCurveTo(cx - r * 1.3, cy - r * 0.1, cx - r * 0.6, cy - r * 1.2, cx, cy - r * 0.4);
      ctx.bezierCurveTo(cx + r * 0.6, cy - r * 1.2, cx + r * 1.3, cy - r * 0.1, cx, cy + r * 0.9);
      ctx.closePath(); ctx.fillStyle = '#e0557a'; ctx.fill();
    }
    ...
    state.units.forEach(function (u2) {
      ...
      var tw = ctx.measureText(u2.code).width;
  ...
  function statsInto(box) {
    ...
    var h = '<div class="stat-head"><span class="code code-' + u.side + '"' + armyStyle(u.side) + '>' + u.code + '</span>' +
      '<div class="sh-text"><h2>' + u.name + honourMarks(u) + '</h2><span class="sub">Tier ' + u.tier + '</span></div>' +
      '</div>';
  ...
  function drawMachineStats(u, box) {
    ...
    var pr = R.propOf(u);
    var kind = u.cls === 'aircraft' ? 'Aircraft' : pr ? pr.name : 'Vehicle';
    if (u.drone) kind += ' · drone';
    var h = '<div class="stat-head"><span class="code code-' + u.side + '"' + armyStyle(u.side) + '>' + u.code + '</span>' +
      '<div class="sh-text"><h2>' + u.name + honourMarks(u) + '</h2><span class="sub">Tier ' + u.tier + ' · ' + kind + '</span></div>' +
      '</div>';
