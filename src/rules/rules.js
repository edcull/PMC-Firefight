...
(function (root) {
  ...
  var WEAPONS = {
    ...
    lrrp: { p: 'rail', n: 2 }, snipers: { p: 'rail', s: 'pistol' },
  ...
  /* ---------- the men themselves: names, ranks, and who fell ----------
     ...
     count drops the ones who fell are picked out of the living at random. None
     ...
     its crew instead: one commander or pilot, lost when the hull is destroyed.
     Drones, turrets and the swarm's living hulls have nobody aboard. */
  ...
  var BUG_SYL = ['skr', 'xik', 'tch', 'zz', 'kra', 'vix', 'ch', 'ekk', 'rr', 'ix', 'kt', 'shh'];
  ...
  function isInfectedHuman(u) { return u.group === 'Infected Humans'; }
  ...
  function soldierName(u) {
    ...
    if (f === 'bugs' && !isInfectedHuman(u)) return syllables(BUG_SYL, 2, 3) + '-' + (10 + Math.floor(Math.random() * 90));
    return humanName(f === 'rebel');
  }
  // does this machine carry anyone who could be named?
  function crewed(u) {
    if (!isMachine(u)) return true;
    return !u.drone && u.faction !== 'bugs' && !has(u, 'Turret') && !/Turret/.test(u.group || '');
  }
  ...
  function rankFor(u, i) {
    ...
    if (f === 'bugs') {
      if (isInfectedHuman(u)) return 'Infected';
      if (has(u, 'Overmind')) return 'Overmind';
      return i === 0 && (u.size || 1) > 1 ? 'Brood-alpha' : 'Drone';
    }
    if (f === 'xeno') {
  ...
  function musterMen(u, carried, taken) {
    ...
    u.men = (carried || []).filter(function (m) { return m && m.name && m.fell == null; })
  ...
  /* Bring the named men into line with the model count: one picked at random
     from the living for every model lost, and a fresh one for every model the
     rules gave back (the Endless Tide, a unit returned to the pool). */
  function syncMen(u, turn, taken) {
    if (!u.men) return [];
    var live = u.men.filter(function (m) { return m.fell == null; });
    var want = crewed(u) ? standing(u) : 0, fell = [];
    ...
    while (live.length > want) {
      ...
      m.fell = turn || 0;
      m.fate = isMachine(u) && !u.catastrophic && !isFlying(u) ? 'WIA' : 'KIA';
      fell.push(m);
    }
    if (!isMachine(u)) while (live.length < want) { var n = freshMan(u, taken); u.men.push(n); live.push(n); }
    return fell;
  }
  // who comes back for the next battle: the living, and a hull's wounded crew
  function survivors(u) {
    return (u.men || []).filter(function (m) { return m.fell == null || m.fate === 'WIA'; }).map(function (m) { return { name: m.name, rank: m.rank }; });
  }
  ...
  root.PMC = {
    ...
    soldierName: soldierName, rankFor: rankFor, crewed: crewed, musterMen: musterMen, syncMen: syncMen, survivors: survivors
  };
})(window);
