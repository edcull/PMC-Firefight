...
if (begun) {
  ...
  while (!W.PMC_STATE().over && guard++ < 2000) {
    ...
    if (ins) {
      ...
      if (ins.kind === 'shove' && !globalThis.__told) { globalThis.__told = 1; settle(); console.log('SHOVE', JSON.stringify(ins), 'spot0', JSON.stringify(spots && spots[0]), 'refused', JSON.stringify(W.__lastRefusal), 'still', JSON.stringify(W.__insertionState()), 'seats', W.__seats && W.__seats().join(','), 'mySide', W.__mySide && W.__mySide(), 'active', W.PMC_STATE().activeSide); }
      continue;
    }
