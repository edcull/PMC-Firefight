...
(function (root) {
  ...
  function drawUnit(g, u, opts) {
    ...
    /* The ring (and the morale bar) show the unit's state; the figures take
       their pose from `status`. They part only while a squad gets up off the
       ground as it arrives: posed flat, then kneeling, but it is not broken or
       suppressed, so its ring stays in the army's colour. */
    var rst = opts.ringStatus || st;
    var art = u.art || 'rifle';
