...
(function (root) {
  ...
  var ROLES = {
    ...
    recruit: ['recruit'],
    enforcer: ['riot'],
    irregular: ['irregular', 'irregular2', 'irregular3'],
    penal: ['convict'],
    ...
    rebel: ['rebrifle', 'rebciv', 'rebmolotov'],
    civilian: ['civ1', 'civ2', 'civ3', 'civ4', 'civ5'],
    ...
    insurgent: ['hardlead', 'hardrifle', 'hardrifle'],
  ...
  var KIT = {
    ...
    irregular: { helm: 'scarf', gun: 'battlerifle', pack: 'none', light: true, bandolier: true },
    irregular2: { helm: 'boonie', gun: 'carbine', pack: 'none', light: true, bandolier: true, tint: IRR_OLIVE },
    irregular3: { helm: 'bandana', gun: 'battlerifle', pack: 'std', light: true, band: '#7a2e28', tint: IRR_KHAKI },
    ...
    shocklead: { helm: 'sealed', gun: 'smg', pack: 'charges', bulk: 1, mark: true, vest: true },
    shockbreacher: { helm: 'sealed', gun: 'shotgun', pack: 'charges', bulk: 1, vest: true },
    ...
    commandolead: { helm: 'sealed', gun: 'smg', pack: 'charges', bulk: 1, mark: true, vest: true, nvg: true },
    commandobreacher: { helm: 'sealed', gun: 'shotgun', pack: 'charges', bulk: 1, vest: true, nvg: true },
    ...
    bunder: { bug: 'under', big: 0.46, mz: [22, -12] },
    bhugeunder: { bug: 'hugeunder', big: 0.6, mz: [28, -16] },
    ...
    hardlead: { helm: 'std', forceHelm: true, gun: 'battlerifle', pack: 'none', mark: true, tint: FIELD_OCHRE },
    ...
    deserter: { helm: 'std', gun: 'rifle', armband: 'force', pack: 'none', tint: REBEL_WORN },
    ...
    agitator: { helm: 'turban', gun: 'battlerifle', pack: 'std', tunic: true, wrap: '#d6ceb8', vest: '#1c1e18', sash: '#2e7a3a', mark: true, bulk: 1, tint: HOLY_B },
  ...
  function paintFigure(g, ox, oy, pal, kit, pose, step, s) {
    ...
    var drop = pose === 'kneel' ? 8 : 0;               // how far the body sinks
    ...
    if (kit.mount && pose !== 'prone') {
      ...
      P(-9, -12, 8, 8, pal.dark);                      // folded rear leg
      P(-9, -12, 3, 8, pal.mid);
      P(-11, -4, 10, 4, BOOT);
      P(2, -16, 6, 12, pal.dark);                      // forward knee
      P(2, -16, 3, 12, pal.mid);
      P(2, -16, 6, 3, pal.helm);                       // knee pad
      P(1, -4, 8, 4, BOOT);
    ...
    if (kit.sash) {                                     // a sash, shoulder to hip
      for (var sh2 = 0; sh2 < 6; sh2++) P(tx + tw - 5 - sh2 * 2.4, -41 + drop + sh2 * 3.2, 4, 3.4, kit.sash);
    }
    ...
    if (kit.pouches && !kit.armoured) {                 // light infantry: a green smock and chest rig
    ...
    switch (kit.helm) {
      ...
      case 'turban':                                    // a turban wound round the head, a beard below
        ...
        P(-7, -57 + drop, 15, 8, kit.wrap || '#d6ceb8');
        ...
        P(-9, -52 + drop, 3, 11, kit.wrap || '#d6ceb8'); // the tail down the back
        P(-9, -52 + drop, 1, 11, 'rgba(0,0,0,.2)');
        if (kit.facewrap) {                              // the end of it wound across the face
          P(-5, -47 + drop, 11, 5, kit.wrap || '#d6ceb8');
  ...
  function muzzleArt(kit, pose) {
    ...
    var drop = pose === 'kneel' ? 8 : 0, wy = -32 + drop;
  ...
  function paintBug(g, ox, oy, pal, kit, pose, step, s, dead) {
    ...
    function wings(cx, cy, len, big) {
      ...
      var up = step ? -0.9 : -0.35;                   // two beats of the wing
      var a2 = up + (big ? 0 : 0.1);
    ...
    switch (kind) {
      ...
      case 'oversized': {
        ...
        // the far scythe
        var lift2 = down ? 16 : pose === 'kneel' ? 6 : 0, swing = step ? 3 : 0;
      ...
      case 'hugeunder': {
        ...
        var sink = pose === 'kneel' ? 3 : down ? 5 : 0;
        mound(-2 * zz, 18 * zz, 5 * zz);
        shell(-4 * zz, -7 * zz + sink, 13 * zz, 8 * zz);
        ridges(-4 * zz, -7 * zz + sink, 13 * zz, 8 * zz, 5);
        spikes([[-12 * zz, -13 * zz + sink], [-4 * zz, -15 * zz + sink], [4 * zz, -13 * zz + sink]], 4 * zz);
        head(9 * zz, -8 * zz + sink, 5.5 * zz, 5 * zz, 3, false);
        // jaws coming up out of the earth
        var op = pose === 'stand' ? (step ? 1 : 0.7) : 0.2;
        L([[12 * zz, -10 * zz + sink], [19 * zz, -18 * zz - op * 4 + sink], [23 * zz, -13 * zz + sink]], 2 * zz, BONE);
        L([[12 * zz, -6 * zz + sink], [20 * zz, -4 * zz + sink], [24 * zz, -8 * zz + sink]], 2 * zz, BONE_D);
        // the front of the mound, over its belly
        E(-2 * zz, 0, 19 * zz, 3 * zz, '#4a3d2a');
        E(-6 * zz, -1 * zz, 10 * zz, 1.6 * zz, '#5f4f37');
        break;
      }
      /* ---- spore bugs ---- */
      case 'spitlarva': {
  ...
  function paintXeno(g, ox, oy, pal, kit, pose, step, s, dead) {
    ...
    if (kit.xeno === 'esh') {
      ...
      var lo2 = kneel ? 6 : 0, rank = kit.rank || 'beta';
      ...
      leg(0, hipY, kneel ? -2 : -1, hipY - 7, kneel ? 3 : 1 - sw2, true);
      arm([[-4, by - 7], [-11, by - 9], [-14, by - 15], [-11, by - 21]], 1.9, 0.8, FAR, true);
      if (!dead) E(-12, by - 12, 0.8, 0.8, GL.d || GL.m);
      leg(-4, hipY, kneel ? -12 : -10 - sw2 * 0.5, hipY - 6, kneel ? -15 : -13 - sw2, false);
      // the far arm
      ...
      leg(3, hipY, kneel ? 11 : 9 + sw2 * 0.5, hipY - 6, kneel ? 15 : 13 + sw2, false);
  ...
  var BIGBUG = {
    ...
    bugsandworm: { kind: 'sandworm', h: 84 },
  ...
  function drawBigBug(g, u, opts) {
    ...
    if (kind === 'sandworm') {
      ...
      for (var i = 0; i <= 16; i++) {
        var tt = i / 16;
        var along = -1.15 + tt * 2.1;
        var z = Math.max(0, Math.sin(Math.min(1, tt * 1.25) * Math.PI * 0.62) * 72 * up) + (i % 4 === 2 ? wob : 0);
        pts.push(S(along, Math.sin(tt * 3) * 0.12, z));
      }
      var mound = S(-1.15, 0, 0);
      add(mound.d - 3, function () {
        ellipse(g, mound.x, mound.y, 0.9 * K, 0.4 * K, '#4a3d2a');
        ellipse(g, mound.x - 3, mound.y - 2, 0.62 * K, 0.26 * K, '#6a5a40');
      });
      ...
      if (MOUNTS) MOUNTS.gun = [{ dx: head.x - gp.x, dy: head.y - gp.y - (opts.lift || 0) + base, dir: 1 }];
    } else {
      ...
      if (kind === 'firebeetle') {
        ...
        // the brain under its crest, the crown of horns, eggs on the abdomen
        var bc = S(0.55, 0, 70), br = 0.44 * K;
        add(bc.d + 3, function () {
          if (!dead) ellipse(g, bc.x, bc.y, br * 1.35, br * 1.2, 'rgba(185,140,242,.18)');
          ellipse(g, bc.x, bc.y, br + 1.5, br * 0.86 + 1.5, '#0b0908');
          ellipse(g, bc.x, bc.y, br, br * 0.86, psy.d);
          ellipse(g, bc.x - 1, bc.y - 2, br * 0.9, br * 0.74, psy.m);
          for (var fo = 0; fo < 3; fo++) {
            g.strokeStyle = psy.d; g.lineWidth = 1.2;
            g.beginPath(); g.moveTo(bc.x - br * 0.7, bc.y - br * 0.4 + fo * br * 0.35);
            g.quadraticCurveTo(bc.x, bc.y - br * 0.6 + fo * br * 0.35, bc.x + br * 0.7, bc.y - br * 0.35 + fo * br * 0.35); g.stroke();
          }
          ellipse(g, bc.x - br * 0.35, bc.y - br * 0.45, br * 0.3, br * 0.18, psy.l);
          for (var hn = 0; hn < 5; hn++) {
            var hx = bc.x + (hn - 2) * br * 0.4, hy = bc.y - br * 0.7 + Math.abs(hn - 2) * 2;
            poly(g, [[hx - 2.5, hy + 2], [hx + (hn - 2) * 1.5, hy - 12], [hx + 2.5, hy + 2]], SH.dk);
          }
        });
  ...
  function sprite(side, art, i, pose, step, scale, shade, mountKind) {
    ...
    var s = SU * sq * SPRITE_RES;
    var w = Math.ceil(SPR.w * s), h = Math.ceil(SPR.h * s);
    var ma = kit.bug || kit.xeno ? bugMuzzle(kit, pose) : muzzleArt(kit, pose);
    var ox = Math.round(SPR.ox * s), oy = Math.round(SPR.oy * s);
    ...
    var pal = PALETTE[side] || PALETTE.A;
  ...
  var EMPLACEMENT = {
    ...
    rebelgun: function (g, cx, cy, s, pal, dir) {
      ...
      P(-6, -20, 2, 14, '#50594c');
      // road wheels either side of the cradle
      ellipse(g, cx - 9 * s * (dir || 1), cy - 7 * s, 6 * s, 6 * s, '#20241f');
      ellipse(g, cx - 9 * s * (dir || 1), cy - 7 * s, 3 * s, 3 * s, '#4a5247');
      ellipse(g, cx + 7 * s * (dir || 1), cy - 6 * s, 5 * s, 5 * s, '#20241f');
  ...
  /* The SAM team's launcher: a twin rack of missile tubes laid up at the
     sky on a pedestal tripod, with an IFF dish and a sight between them. */
  EMPLACEMENT.samlauncher = function (g, cx, cy, s, pal, dir) {
    var P = pieceP(g, cx, cy, s, dir);
    ellipse(g, cx + 2 * s * (dir || 1), cy, 17 * s, 6 * s, 'rgba(12,10,8,.34)');
    tripodBase(P, 13, true);
    P(-3, -22, 6, 9, '#39413a');                     // the pedestal
    P(-7, -24, 14, 4, '#2e3530');                    // the cradle
    for (var t = 0; t < 2; t++) {                    // two tubes laid up at the sky, far one first
      var ox = t ? -1 : 2, oy = t ? 0 : -5, col = t ? '#5a6a48' : '#4a5a3a';
      for (var i = 0; i < 16; i++) {
        P(-12 + ox + i * 1.7, -20 + oy - i * 1.25, 4, 5, col);
        P(-12 + ox + i * 1.7, -20 + oy - i * 1.25, 4, 1.2, '#7a8a62');
      }
      P(-13 + ox + 16 * 1.7, -21 + oy - 16 * 1.25, 3, 5, '#2e3a24');    // the front cap
    }
    P(-4, -32, 5, 5, '#39413a');                     // sight
    P(0, -31, 1.5, 2, '#8fb8cc');
    P(-14, -30, 1.5, 7, '#39413a');                  // the IFF dish on its mast
    P(-18, -33, 8, 2, '#8a9088'); P(-17, -34, 6, 1, '#b0b6ae');
    P(-26, -8, 8, 7, pal.dark); P(-26, -8, 8, 2, pal.mid);          // the battery box in unit colours
  };
  ...
  function flyLift(u) {
    if (!u || !u.art) return 0;
    var spec = hullSpec(u.art), sk = skimOf(u.art);
    return spec && spec.fly ? ELEV * (sk ? sk.hover : spec.fly) : 0;
  }
  ...
  /* ---- the skimmers: Xenotripod craft as hover tanks ----
     A proposal, drawn only while the preview switch is on: the craft as a low,
     faceted ivory hull riding a hand's breadth over the ground on blue
     repulsors, a turret pod on its back with the energy barrel, swept tail
     fins, the army's colour along its edges. Same keys as XENO3D's craft:
     L and W the hull's half length and half width in inches, H its height in
     hull pixels, hover how high it rides (in hill heights; the tri-wings fly
     at 2.2 to 2.8). */
  var XENO_SKIM = false;
  var SKIM = {
    xstrike: { L: 0.8, W: 0.78, H: 5, hover: 0.8, barrels: 1 },
    xstrikehg: { L: 0.88, W: 0.86, H: 5.5, hover: 0.8, barrels: 2 },
    xstrikeadv: { L: 0.96, W: 0.94, H: 6, hover: 0.85, barrels: 2, prongs: true },
    xrecon: { L: 0.62, W: 0.58, H: 4, hover: 1.0, barrels: 1, eye: true, small: true },
    xtelecraft: { L: 0.8, W: 0.8, H: 5.5, hover: 0.8, barrels: 1, ring: true },
    xshieldcraft: { L: 0.8, W: 0.8, H: 5.5, hover: 0.8, barrels: 1, dome: true }
  };
  function skimOf(art) { return XENO_SKIM && SKIM[art] ? SKIM[art] : null; }
  ...
  function drawXenoMachine(g, u, opts) {
    ...
    var base = (opts.lift || 0) - (opts.hop || 0);
    var sk = spec.kind === 'craft' ? skimOf(u.art) : null;
    var fly = hs.fly && !dead ? ELEV * (sk ? sk.hover : hs.fly) : 0;
    var lift = base + fly;
    var pal = PALETTE[u.paint || u.side] || PALETTE.A;
    ...
    if (sk) return drawSkimmer();
    /* A skimmer, drawn in hull pixels about its own axis: a faceted lower hull,
       a turret pod on the deck, the barrel(s) forward, fins at the tail. */
    function drawSkimmer() {
      var L = sk.L, Wd2 = sk.W, zb = fly + (dead ? 0 : Math.sin(tnow / 380) * 1.2), H = m(sk.H), zd = zb + H;
      var BL = dead ? { m: '#3e3c38', l: '#4a4843', h: 'rgba(0,0,0,0)' } : { m: '#6ebeff', l: '#e4f4ff', h: 'rgba(110,190,255,.3)' };
      /* the hull's planform, nose first, going round to starboard: a slim
         hull with a pointed nose, and thin swept wings either side of it */
      var PB = [[1.2 * L, 0], [0.5 * L, 0.32 * Wd2], [-0.88 * L, 0.36 * Wd2], [-L, 0.22 * Wd2], [-L, -0.22 * Wd2], [-0.88 * L, -0.36 * Wd2], [0.5 * L, -0.32 * Wd2]];
      var PT = PB.map(function (q) { return [q[0] * 0.9 - 0.02 * L, q[1] * 0.78]; });
      var NP = PB.length;
      function faceCol(nx, ny) {
        var lit = -nx * 0.6 + ny * 0.2;
        return lit > 0.25 ? WH.lt : lit > -0.25 ? WH.md : WH.dk;
      }
      // a frustum from planform A at za up to planform B at zb2, sides toward the eye, then its top
      function frustum(A, B, za, zb2, top, t0) {
        t0 = t0 || 0;
        var faces = [];
        for (var i = 0; i < A.length; i++) {
          var j = (i + 1) % A.length;
          var ex = A[j][0] - A[i][0], ey = A[j][1] - A[i][1];
          // the outward normal in the craft's frame, turned into the world
          var ln = Math.sqrt(ex * ex + ey * ey) || 1, onx = ey / ln, ony = -ex / ln;
          var wx = cos * onx - sin * ony, wy = sin * onx + cos * ony;
          if (wx + wy <= -0.02) continue;
          faces.push({ d: wx + wy, col: faceCol(wx, wy), pts: [S(A[i][0] + t0, A[i][1], za), S(A[j][0] + t0, A[j][1], za), S(B[j][0] + t0, B[j][1], zb2), S(B[i][0] + t0, B[i][1], zb2)] });
        }
        faces.sort(function (p1, p2) { return p1.d - p2.d; }).forEach(function (fc) { path(fc.pts, fc.col); stroke(fc.pts, 1, WH.seam, true); });
        var cap = B.map(function (q) { return S(q[0] + t0, q[1], zb2); });
        path(cap, top || WH.lt); stroke(cap, 1, WH.seam, true);
        return cap;
      }
      // the shadow, and the repulsor wash on the ground under it
      if (!dead) {
        path(PB.map(function (q) { return S(q[0] * 1.05, q[1] * 1.05, 0); }), 'rgba(12,10,8,.3)');
        var gc = S(0, 0, 0);
        ellipse(g, gc[0], gc[1], m(10) * L, m(4.2) * L, 'rgba(110,190,255,.12)');
      }
      // the belly: a darker skirt under the hull, and the repulsor pads glowing under it
      var PU = PB.map(function (q) { return [q[0] * 0.8, q[1] * 0.7]; });
      if (!dead) {
        [[0.4 * L, 0.16 * Wd2], [0.4 * L, -0.16 * Wd2], [-0.6 * L, 0.7 * Wd2], [-0.6 * L, -0.7 * Wd2]].forEach(function (q) {
          var pp = S(q[0], q[1], zb - m(0.5));
          var fl3 = 0.8 + 0.2 * Math.sin(tnow / 90 + q[0] * 7 + q[1] * 3);
          halo(pp, m(3.4) * fl3, BL.h);
          ellipse(g, pp[0], pp[1], m(1.6), m(0.8), BL.m);
          ellipse(g, pp[0], pp[1] - m(0.2), m(0.8), m(0.4), BL.l);
        });
      }
      frustum(PU, PB, zb - m(1.5), zb + m(1.5), WH.dk);
      // the side a fin is on: toward the eye or away
      function finSide(sd) { var wx = -sin * sd, wy = cos * sd; return wx + wy; }
      // a wing: a thin plate swept back from the hull's flank, its leading edge in the army's colour
      var wz = zb + H * 0.3;
      function wingPlate(sd) {
        // thrown down and out like a tri-wing's lower pair: the tip well below the root
        var dz = m(sk.small ? 3 : 4.5);
        var top = [S(0.35 * L, sd * 0.3 * Wd2, wz), S(-0.52 * L, sd * Wd2, wz - dz), S(-0.86 * L, sd * 0.96 * Wd2, wz - dz), S(-0.72 * L, sd * 0.34 * Wd2, wz)];
        var under = top.map(function (q) { return [q[0], q[1] + m(1.2)]; });
        path([top[0], top[1], under[1], under[0]], WH.dk);                // the leading edge's thickness
        path([top[1], top[2], under[2], under[1]], WH.sh);
        path(top, finSide(sd) > 0 ? WH.lt : WH.md); stroke(top, 1, WH.seam, true);
        stroke([S(0.1 * L, sd * 0.36 * Wd2, wz - dz * 0.1), S(-0.6 * L, sd * 0.82 * Wd2, wz - dz * 0.8)], 1, WH.dk);   // a panel line
        stroke([top[0], top[1]], m(0.8), GLO.m);
        if (!dead) { stroke([top[0], top[1]], m(0.3), GLO.l); ellipse(g, top[1][0], top[1][1], m(0.9), m(0.8), GLO.l); }
        return top;
      }
      var wingTips = {};
      [-1, 1].forEach(function (sd) { if (finSide(sd) < 0) wingTips[sd] = wingPlate(sd)[1]; });
      var deck = frustum(PB, PT, zb + m(1.5), zd);
      [-1, 1].forEach(function (sd) { if (finSide(sd) >= 0) wingTips[sd] = wingPlate(sd)[1]; });
      // the army's colour down the nose
      var LE = [deck[1], deck[0], deck[NP - 1]];
      stroke(LE, m(0.8), GLO.m);
      if (!dead) { stroke(LE, m(0.3), GLO.l); halo(deck[0], m(1.6), GLO.h); }
      // a panel seam down the deck
      stroke([S(0.6 * L, 0, zd), S(-0.7 * L, 0, zd)], 1, WH.dk);
      // the turret pod and its barrels; the barrels go behind the pod when the nose points away
      var tz = zd, th = m(sk.small ? 3.5 : 5), tr = sk.small ? 0.8 : 1;
      var PP = [[0.4 * L * tr, 0], [0.12 * L, 0.22 * Wd2 * tr], [-0.36 * L, 0.22 * Wd2 * tr], [-0.5 * L, 0], [-0.36 * L, -0.22 * Wd2 * tr], [0.12 * L, -0.22 * Wd2 * tr]];
      var PPt = PP.map(function (q) { return [q[0] * 0.8, q[1] * 0.74]; });
      var muz = [];
      function barrels() {
        var n = sk.barrels || 1;
        for (var bi = 0; bi < n; bi++) {
          var bs = n === 1 ? 0 : (bi ? 1 : -1) * 0.12 * Wd2, bz = tz + th * 0.5;
          var b0 = S(0.1 * L, bs, bz), b1 = S((sk.small ? 1.05 : 1.3) * L, bs, bz);
          stroke([b0, b1], m(2.2), dead ? WH.seam : '#123a6e');
          stroke([b0, b1], m(1.3), dead ? WH.dk : '#3f9be8');
          stroke([b0, S((sk.small ? 1.0 : 1.24) * L, bs, bz + m(0.4))], m(0.5), dead ? WH.md : '#bfe6ff');
          [0.55, 0.85, 1.1].forEach(function (t3) {
            if (sk.small && t3 > 1) return;
            var c3 = S(t3 * L, bs, bz);
            ellipse(g, c3[0], c3[1], m(1), m(1.1), dead ? WH.dk : '#6ebeff');
          });
          if (!dead) { halo(b1, m(2.2), 'rgba(110,190,255,.35)'); ellipse(g, b1[0], b1[1], m(0.8), m(0.7), '#e4f4ff'); }
          muz.push(b1);
        }
      }
      var away = cos + sin < 0;
      /* the third wing: a tall swept blade standing straight up off the spine,
         the tri-wing's dorsal wing, its leading edge burning in the army's colour */
      function dorsal() {
        var fh = m(sk.small ? 9 : 13), bw = 0.05 * Wd2;
        var bl = [S(0.05 * L, 0, zd), S(-0.95 * L, 0, zd + fh), S(-1.1 * L, 0, zd + fh * 0.9), S(-0.95 * L, 0, zd)];
        var side = finSide(1) > 0 ? 1 : -1;
        var edge2 = bl.map(function (q, i) { return i === 1 || i === 2 ? q : S(i ? -0.95 * L : 0.05 * L, side * bw, zd); });
        path(edge2, WH.dk);
        path(bl, WH.md); stroke(bl, 1, WH.seam, true);
        stroke([S(-0.25 * L, 0, zd + fh * 0.25), S(-0.9 * L, 0, zd + fh * 0.8)], 1, WH.lt);      // a panel line
        stroke([bl[0], bl[1]], m(0.8), GLO.m);
        if (!dead) { stroke([bl[0], bl[1]], m(0.3), GLO.l); ellipse(g, bl[1][0], bl[1][1], m(0.9), m(0.8), GLO.l); }
      }
      if (!away) dorsal();
      if (away) barrels();
      frustum(PP, PPt, tz, tz + th, WH.lt);
      // the cockpit: a blue crystal set in the front of the pod
      var cq = [S(0.36 * L * tr, 0, tz + th * 0.35), S(0.14 * L, 0.2 * Wd2 * tr, tz + th * 0.7), S(0.0 * L, 0, tz + th + m(0.6)), S(0.14 * L, -0.2 * Wd2 * tr, tz + th * 0.7)];
      if (!away) { path(cq, dead ? '#2a2826' : '#3f9be8'); stroke(cq, 1, dead ? WH.seam : '#123a6e', true); if (!dead) path([cq[0], cq[1], cq[2]], '#9fd6ff'); }
      if (!away) barrels();
      if (sk.prongs) {
        // blades forward off the shoulders, the rocket pylons
        [-1, 1].forEach(function (sd) {
          var pr = [S(-0.52 * L, sd * Wd2, wz - m(0.8)), S(0.3 * L, sd * 0.98 * Wd2, wz), S(-0.66 * L, sd * 0.86 * Wd2, wz - m(0.8))];
          path(pr, WH.lt); stroke(pr, 1, WH.seam, true);
          stroke([pr[0], pr[1]], m(0.7), GLO.m);
          if (!dead) { ellipse(g, pr[1][0], pr[1][1], m(0.8), m(0.7), GLO.l); mountAt('rocket', pr[1]); }
        });
      }
      if (sk.dome) {
        var dm = S(-0.06 * L, 0, tz + th + m(2.6));
        ellipse(g, dm[0], dm[1], m(4.4), m(3.4), dead ? '#4a4843' : hexA(gc1, 0.35));
        ellipse(g, dm[0] - m(1.1), dm[1] - m(1.1), m(1.3), m(0.9), dead ? '#55524c' : 'rgba(255,255,255,.55)');
        if (!dead) ellipseRing(g, dm[0], dm[1], m(4.4), m(3.4), GLO.m);
      }
      if (sk.eye) { var ey = S(1.02 * L, 0, zb + H * 0.5); halo(ey, m(4), GLO.h); ellipse(g, ey[0], ey[1], m(1.8), m(1.5), GLO.m); ellipse(g, ey[0] - 1, ey[1] - 1, m(0.7), m(0.6), GLO.l); }
      if (sk.ring) {
        var rp = [];
        for (var ri3 = 0; ri3 <= 24; ri3++) { var ra3 = ri3 / 24 * Math.PI * 2; rp.push(S(-0.1 * L + Math.cos(ra3) * 0.62 * L, Math.sin(ra3) * 0.62 * L, zd + th + m(3))); }
        stroke(rp, m(1.8), WH.dk);
        stroke(rp, m(1), BL.m);
      }
      if (away) dorsal();
      if (!dead) muz.forEach(function (mp) { mountAt('gun', mp); mountAt('mg', mp); });
      return { lift: lift, hgt: zd - zb + th + m(sk.dome ? 8 : sk.ring ? 6 : 8) };
    }
  ...
  function drawMachineBody(g, u, opts) {
    ...
    function hexFlank(fr, a0, a1, bSide, z0, h) {
      var n = 6, step = (a1 - a0) / n;
      for (var i = 0; i < n; i++) {
        for (var r = 0; r < 2; r++) {
      ...
      switch (st) {
        ...
        case 'jet': case 'hybrid': {
          ...
          part(0, 0, function () {
            ...
            tailFin(-L * 0.3, z + H * 0.7, hyb ? 14 : 16, true);
    ...
    function drawMech() {
      ...
      function heavyTorso() {
        ...
        /* the cockpit: one wide pane of glass set into the top of the chest,
           framed in dark metal */
        if (fwd > -0.1) {
  ...
  function muzzles(u, status) {
    ...
    var pose = status === 'broken' ? 'prone' : status === 'suppressed' ? 'kneel' : 'stand';
  ...
  function drawUnit(g, u, opts) {
    ...
    if (around) {
      var ringCol0 = st === 'broken' ? '#e0557a' : st === 'suppressed' ? '#e08a3a' : pal.light;
    ...
    var ringCol = st === 'broken' ? '#e0557a' : st === 'suppressed' ? '#e08a3a' : pal.light;
    ...
    var scale = MODEL;
    var pose = st === 'broken' ? 'prone' : st === 'suppressed' ? 'kneel' : 'stand';
    ...
    if (u.sp > 0 && opts.morale) {
      ...
      rect(g, p.x - w / 2, head, filled, bh, st === 'broken' ? '#d1476b' : '#c9762f');
    }
    if (u.marked) {
  ...
  root.PMCIso = {
    K: K, ART: A, PIXEL: PIXEL, ELEV: ELEV, PIXW: PIXW, PIXH: PIXH, W: W, H: H,
    ...
    // the Xenotripod skimmer proposal: on only to preview it
    previewXenoSkimmers: function (on) { XENO_SKIM = !!on; },
    ...
    headroom: function (models, status, unit) {
      ...
      return headroom(n, status === 'broken' ? 'prone' : status === 'suppressed' ? 'kneel' : 'stand',
