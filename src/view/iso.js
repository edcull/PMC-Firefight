...
(function (root) {
  ...
  var FLOOR = {
    ...
    // the ground under a desert mesa: red sandstone scree (the rock itself is a prop)
    mesa: { ramp: ['#4a2618', '#5e321f', '#733f27', '#884c2f'], fleck: ['#a8623c', '#2e170e'], rim: '#2a150c' },
  ...
  var FIT = {
    ...
    mesa:     { r: 3, p: -0.14, d: 0.18, n: -0.04, tint: '#8a4f31', tw: 0.4 },
  ...
  function buildProps(terrain, objectives, seed, planet) {
    ...
    terrain.forEach(function (r, ri) {
      ...
      if (r.kind === 'mesa') {
        // a desert mesa: the whole rock stands as one prop, sorted in with the rest
        props.push({ kind: 'mesa', x: r.x, y: r.y, w: r.w, h: r.h, piece: r, height: a(11 + rand() * 6), seed: (rand() * 9999) | 0 });
        return;
      }
    ...
    var BOX = { building: 1, bunker: 1, highwall: 1, objective: 1, mesa: 1 };
  ...
  function drawProp(g, pr, lift, cutaway) {
    ...
    switch (pr.kind) {
      ...
      case 'mesa': {
        /* Sheer sandstone walls round the piece's own outline, banded in strata
           and split by vertical joints, under a flat, sun-baked top. Only the
           faces turned to the viewer are drawn; the top covers the rest. */
        var mr = rng(pr.seed), MH = pr.height, pc = pr.piece;
        var medges = hillOutline(pc);
        var mfoot = medges.map(function (e) { return sp2(e.A[0], e.A[1], lift); });
        for (var ms2 = 0; ms2 < 4; ms2++) {
          var mo = 4 + ms2 * 4;
          poly(g, mfoot.map(function (q) { return [q[0] + mo, q[1] + mo * 0.55]; }), 'rgba(40,18,8,.14)');
        }
        medges.filter(function (e) { return e.nx + e.ny > 0.02; })
          .sort(function (e1, e2) { return (e1.mx + e1.my) - (e2.mx + e2.my); })
          .forEach(function (e) {
            var lit = Math.max(0, Math.min(1, (e.ny - e.nx + 1) / 2));
            var A = sp2(e.A[0], e.A[1], lift), B = sp2(e.B[0], e.B[1], lift);
            var AT = [A[0], A[1] - MH], BT = [B[0], B[1] - MH];
            var top = lit > 0.5 ? '#d08452' : '#94502f', mid = lit > 0.5 ? '#a8603a' : '#6e3822', foot = lit > 0.5 ? '#7a4127' : '#4a2415';
            poly(g, [A, B, BT, AT], vgrad(g, Math.min(AT[1], BT[1]), Math.max(A[1], B[1]), [top, mid, foot]));
            var len = Math.hypot(B[0] - A[0], B[1] - A[1]);
            // strata: level bands the length of the face
            for (var k8 = 1; k8 < 5; k8++) {
              var up8 = MH * k8 / 5 + (mr() - 0.5) * 2;
              edgeLine(g, [A[0], A[1] - up8], [B[0], B[1] - up8], k8 & 1 ? 'rgba(60,24,10,.45)' : 'rgba(240,180,130,.22)', 1);
            }
            // vertical joints, and the odd fallen block at the foot
            for (var j8 = 0; j8 < len / 8; j8++) {
              var f8 = mr(), jx = A[0] + (B[0] - A[0]) * f8, jy = A[1] + (B[1] - A[1]) * f8;
              edgeLine(g, [jx, jy - MH * (0.1 + mr() * 0.3)], [jx + (mr() - 0.5) * 2, jy - MH * (0.55 + mr() * 0.45)], 'rgba(40,16,6,.55)', 1);
              if (mr() > 0.6) { rect(g, Math.round(jx), Math.round(jy - 2), 3 + (mr() * 3 | 0), 2, lit > 0.5 ? '#b26a42' : '#6e3822'); }
            }
          });
        // the flat top, and its lit far rim
        var mtop = medges.map(function (e) { return sp2(e.A[0], e.A[1], lift + MH); });
        poly(g, mtop, '#b9744a');
        // wind-scoured caprock: patches of lighter and darker stone, grit, and a crack or two
        var tarea = pc.w * pc.h;
        for (var tb = 0; tb < tarea / 6; tb++) {
          var tb0 = spotIn(pc, 0.8, mr), tbs = sp2(tb0.x, tb0.y, lift + MH);
          ellipse(g, tbs.x, tbs.y, a(0.8 + mr() * 1.4), a(0.4 + mr() * 0.6), mr() > 0.5 ? 'rgba(230,170,120,.35)' : 'rgba(110,55,30,.3)');
        }
        for (var tq = 0; tq < tarea * 4; tq++) {
          var tp = spotIn(pc, 0.3, mr), ts = sp2(tp.x, tp.y, lift + MH);
          dot(g, ts.x, ts.y, mr() > 0.5 ? '#dfa574' : '#7e4428', 1);
        }
        for (var tc = 0; tc < 2 + tarea / 25; tc++) {
          var c0 = spotIn(pc, 0.8, mr), cang = mr() * Math.PI, clen = 1 + mr() * 2.5;
          edgeLine(g, sp2(c0.x, c0.y, lift + MH), sp2(c0.x + Math.cos(cang) * clen, c0.y + Math.sin(cang) * clen, lift + MH), 'rgba(70,30,14,.6)', 1);
        }
        medges.forEach(function (e) {
          var A2 = sp2(e.A[0], e.A[1], lift + MH), B2 = sp2(e.B[0], e.B[1], lift + MH);
          edgeLine(g, A2, B2, e.nx + e.ny < 0.05 ? '#e9ae7c' : '#f0c090', 1);
        });
        break;
      }
      case 'vent': {
