    ...
    function drawCraft() {
      ...
      function tailFin(p, zz, hh, twin, fanR) {
        (twin ? [-1, 1] : [0]).forEach(function (sd) {
          ...
          var finCol = sd <= 0 ? mixc(hull, lit, 0.5) : hull;
          poly(g, [b0, b1, t1, t0], finCol);
          if (CAMO) camoOn([b0, b1, t1, t0], finCol, zz + hh * 0.4, false, hh);   // the fin carries the pattern too
        ...
        case 'apache': case 'apacherk': case 'hind': case 'hindrk': {
          ...
          [-1, 1].forEach(function (sd) {
            part(-L * 0.02, sd * w * 2.6, function () {
              // the stub wings carrying the rear fans ride level with the top of the hull
              var wz = z + H - 2;
