...
(function (root) {
  ...
  function animates(u) {
    ...
    return (ROLES[u.art] || []).some(function (r) { return shieldAnimated(KIT[r]) || cloakedKit(KIT[r]); });
  }
  function sprite(side, art, i, pose, step, scale, shade, mountKind) {
    ...
    var key = side + '|' + role + '|' + pose + '|' + step + '|' + sq + '|' + shade + (mountKind ? '|' + mountKind : '') + (shieldy ? '|sp' + SHIELD_PHASE : '');
    var c = sprites[key];
    if (c) return c;
