...
(function (root) {
  ...
  function paintXeno(g, ox, oy, pal, kit, pose, step, s, dead) {
    ...
    if (kit.xeno === 'esh') {
      ...
      var lo = kneel ? XENO_LOW : 0;                   // hunkered down, belly to the ground
      var st = step ? 1 : 0;
      // the far leg
      ...
      var lo2 = kneel ? XENO_LOW : 0, rank = kit.rank || 'beta';   // hunkered: the body let down to the ground
  ...
  function bugMuzzle(kit, pose) {
    var m = kit.mz || [12, -10];
    if (kit.xeno && pose === 'kneel') return [m[0], m[1] + XENO_LOW];
