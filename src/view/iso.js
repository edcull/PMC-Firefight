  ...
  function paintXeno(g, ox, oy, pal, kit, pose, step, s, dead) {
    ...
    if (kit.xeno === 'esh') {
      ...
      if (wpn2 === 'launcher') {                      // Gamma: a portal gun — the orb goes in here and comes out there
        ...
        for (var gc2 = 0; gc2 < 3; gc2++) {                                      // cooling vents down the barrel
          var cx2 = 18.6 + gc2 * 2.6, cy2 = by - 10.8 - gc2 * 0.2;
          E(cx2, cy2, 0.8, 2.8, dead ? '#2c2a26' : PB.d, -0.1);
          E(cx2, cy2, 0.42, 2.1, dead ? '#3a3834' : PB.m, -0.1);
        }
  ...
  function drawXenoMachine(g, u, opts) {
    ...
    if (spec.kind === 'craft') {
      ...
      var barrel = function () {
        ...
        (spec.support ? [0.8] : [0.62, 0.8, 0.98]).forEach(function (t3) {              // cooling vents along it, inside its width
          var c3 = S(t3 * Lc, 0, zb);
          ellipse(g, c3[0], c3[1], m(0.62 * gk), m(0.5 * gk), dead ? WH.seam : '#0b2d5c');
          ellipse(g, c3[0], c3[1], m(0.42 * gk), m(0.32 * gk), dead ? WH.dk : '#6ebeff');
        });
            ...
            gun = function () {
              if (st.stubGun) {
                ...
                if (st.energyGun) {
                  ...
                  if (!dead) sEllipse(bt[0], bt[1], 1.5, 1.3, 'rgba(190,245,255,.95)');
                }
                return;
              }
    ...
    function armWeapon(type, fr, a0, b, z, reach, sc, across) {
      ...
      switch (type) {
        ...
        case 'rail': {
          ...
          var rw = 2.4 * sc, rt = barrel(fr, a0, reach, b, z, rw, 'rail', { col: '#141b22', lit: '#7fd8e8' });
