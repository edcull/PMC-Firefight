...
(function (root) {
  ...
  /* How many dice a machine repairs with: one for each point of Structure it
     has left — so none at all once the last is gone, and it cannot be repaired.
     Nanobots (p. 89) roll for the whole Structure, not for what is left of it. */
  function repairDice(u) {
    return campFlag(u, 'nanobots') ? u.str : Math.max(0, u.str - (u.damage || 0));
  }
  // Rally phase: a machine patches itself up. Jammers make it harder.
  function repair(state, u) {
    if (!u.damage) return null;
    var dice = repairDice(u), need = jammedNearby(state, u) ? 5 : 4;
    if (!dice) {
      return { dice: 0, need: need, rolls: [], fixed: 0, before: u.damage, after: u.damage, none: true,
        text: u.label + ' has no Structure left to repair with (' + u.damage + ' of ' + u.str + ').' };
    }
    var rolls = [], fixed = 0;
